        ...
        if (!phoneResolution) {
            ...
            data.forEach((product, i) => {
                ...
                tr.innerHTML = `
                    ...
                    <td class="py-3 px-1 min-w-16 h-8 text-base font-normal border-white/40 border-r-2">${product.date}</td>
                    ...
                    <div class="flex items-center absolute inset-0 z-10 bg-[#09041C]/0 hover:bg-[#09041C]/70 py-1 text-white/0 hover:text-white transition-all ${i === 0 ? 'rounded-b-3xl' : ''}">
                        <p class="h-fit w-full text-center transition-all">${product.editDate != '' ? 'Ultima edicion ' + product.editDate : 'Sin editar.'} ${product.isEditable ? '' : 'Ya se registro una salida del producto, no se puede editar'}</p>
                    </div>
                `;
                ...
                table.innerHTML = `
                    <tbody class="w-full">
                        ...
                        <tr class="align-middle odd:bg-black/10 even:bg-black/0">
                            ...
                            <td class="w-1/2 text-base font-semibold text-center pt-2 pb-2">${product.date}</td>
                        </tr>
                        ...
                        <tr class="align-middle border-t-2 odd:bg-black/10 even:bg-black/0">
                            <td colspan="2" class="text-white font-semibold bg-[#09041C]/20 rounded-b-3xl">
                                <button ${product.isEditable ? '' : 'disabled'} class="edit-btn flex flex-row items-center justify-center h-8 w-full my-2 px-2 gap-2 mx-auto disabled:opacity-40 hover:scale-110 transition-all">
                                    ...
                                    <p class="max-h-8 text-sm text-white text-center">${product.editDate != '' ? 'Ultima edicion ' + product.editDate : 'Sin editar.'} ${product.isEditable ? '' : 'No se puede editar'}</p>
                                </button>
                            </td>
                        </tr>
                    </tbody>
...
const infoExtractor = (container, purpose) => {
    switch (purpose) {
        case 'edit':
            ...
            const unitEdit = container.children[6].children[1].value;
            const unitPriceEdit = container.children[6].children[0].children[0].value;
            const currencyEdit = container.children[5].children[1].value;
            const alertAmountEdit = container.children[7].children[0].value;
            const descriptionEdit = container.children[8].children[0].value;
            const editDate = new Date().toISOString().split('T')[0].split('-').reverse().join('/');
            const totalPriceEdit = (unitPriceEdit * quantityEdit).toString();
            const alertAmountsEdit = [
                alertAmountEdit,
                (alertAmountEdit * 2).toString(),
                (alertAmountEdit * 3).toString()
            ];
        ...
        case 'add':
            ...
            const alertAmounts = [
                alertAmount,
                (alertAmount * 2).toString(),
                (alertAmount * 3).toString()
            ];
...
tablesContainer.addEventListener('click', e => {
    ...
    const productObject = productsArray.find(product => product.code === productCode && product.date === productEntryDate);
